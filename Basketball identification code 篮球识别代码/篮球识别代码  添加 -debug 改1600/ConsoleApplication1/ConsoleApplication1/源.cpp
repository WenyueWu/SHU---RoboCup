...
void NIfunction(int, void*)
{
	...
	{
		if ((g_c8BitDepth.at<uchar>(i, j) >= (1.0 * 1200 / 7000 * 255)) || (g_c8BitDepth.at<uchar>(i, j) <= 0))
		{
...
double redde(Mat asrc){
	...
	for (int i = 0; i < num_row; i += 2)
	{
		...
		//	uchar*outData = mask.ptr<uchar>(i);
		for (int j = 0; j < num_col; j += 2){
			if ((curr_r_hue[j] <= 12 && curr_r_hue[j] >= 0) && (curr_r_satur[j]>60 && curr_r_satur[j]<255) && (curr_r_value[j]>46 && curr_r_value[j]<200))//找颜色
			{
				count_red1++; count++;
			}
			else if ((curr_r_hue[j] <= 180 && curr_r_hue[j] >= 156) && (curr_r_satur[j]>60 && curr_r_satur[j]<255) && (curr_r_value[j]>46 && curr_r_value[j]<200))
			{
				count_red2++; count++;
			}
			else  count++;
...
void picprocess(Mat asrc, Mat depth, int data)
{
	...
	int j = -1; double index = 0.05, temp = 0;
	...
	for (int unsigned i = 0; i < contours.size(); i++)
	{
			...
			((center[i].x + radius[i]) <srcImage.cols) && ((center[i].y + radius[i]) < srcImage.rows) && (radius[i]>10))
		{
			...
			if (a == 1)
			{
				...
				blue = bluede(imgROI);
				red = redde(imgROI);
				if ((blue >= index) && (blue>0.00) && (red >= temp) && ((red>0.03)) && (red<0.35)) {
					index = blue;
					temp = red;
					j = i;
				}
				...
				red = redde(imgROI);
				if ((yellow >= index) && (yellow>0.08) && (red >= temp) && (red>0.05) && (red<0.35)) {
					index = yellow;
					temp = red;
					j = i;
				}
